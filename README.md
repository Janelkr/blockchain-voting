...
Разворачивает смарт-контракт в блокчейне (локальная сеть или тестнет). Создаёт экземпляр контракта с заданными кандидатами и выводит его адрес, который используется в интерфейсе.

**Как работает:**  
1. Берёт имена кандидатов (`["Alice", "Bob", "Charlie"]`).  
2. Компилирует и деплоит контракт `Voting`.  
3. Выводит адрес, по которому контракт доступен.
...
Настраивает среду разработки Hardhat: версию Solidity, сети для деплоя (локальная `hardhat`, тестовая `sepolia`), RPC-узлы и приватные ключи для подписи транзакций.

**Основные параметры:**  
- `solidity: "0.8.19"` – версия компилятора.  
- `networks.sepolia` – настройки для деплоя на Sepolia: URL узла и приватный ключ аккаунта (берётся из `.env`).
